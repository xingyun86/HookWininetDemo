...
void CDlgRequest::LogHttpResponseData(LPVOID lpBuffer, DWORD dwNumberOfBytesToRead)
{
	char szFileName[MAX_PATH] = { 0 };
	sprintf(szFileName, "%lld.txt", time(nullptr));
	FILE *f = fopen(szFileName, "w");
	if (f)
	{
		fwrite(lpBuffer, 1, dwNumberOfBytesToRead, f);
		fclose(f);
	}
}
void CDlgRequest::LogHttpResponseData(LPINTERNET_BUFFERSA lpBuffersOut)
{
	char szFileName[MAX_PATH] = { 0 };
	sprintf(szFileName, "%lld.txt", time(nullptr));
	FILE* f = fopen(szFileName, "w");
	if (f)
	{
		LPINTERNET_BUFFERSA lp = lpBuffersOut;
		do
		{
			fwrite(lp->lpvBuffer, 1, lp->dwBufferLength, f);
		} while (lp->Next);
		fclose(f);
	}
}
void CDlgRequest::LogHttpResponseData(LPINTERNET_BUFFERSW lpBuffersOut)
{
	char szFileName[MAX_PATH] = { 0 };
	sprintf(szFileName, "%lld.txt", time(nullptr));
	FILE* f = fopen(szFileName, "w");
	if (f)
	{
		LPINTERNET_BUFFERSW lp = lpBuffersOut;
		do
		{
			fwrite(lp->lpvBuffer, 1, lp->dwBufferLength, f);
		} while (lp->Next);
