...
PFN_InternetReadFileExA CWininetHook::s_pfnInternetReadFileExA = InternetReadFileExA;
PFN_InternetReadFileExW CWininetHook::s_pfnInternetReadFileExW = InternetReadFileExW;
...
void CWininetHook::Init()
{
	s_pfnLdrLoadDll = (PFN_LdrLoadDll)::GetProcAddress(::GetModuleHandle(TEXT("NTDLL.DLL")), "LdrLoadDll");
	s_pfnLdrGetProcedureAddress = (PFN_LdrGetProcedureAddress)::GetProcAddress(::GetModuleHandle(TEXT("NTDLL.DLL")), "LdrGetProcedureAddress");
...
void CWininetHook::ReplaceIATEntryForAll(LPCSTR lpszDllName, LPVOID pfnCurrent, LPVOID pfnNew)  
{
	...
	if (hProcess != NULL)
	{
		::EnumProcessModules(hProcess, phModule, 0, &cbNeeded);
		if (cbNeeded > 0)
		{
			phModule = new HMODULE[cbNeeded / sizeof(HMODULE)];
			if (phModule != NULL)
			{
				if (::EnumProcessModules(hProcess, phModule, cbNeeded, &cbNeeded))
				{
					for (int i = 0; i < (int)(cbNeeded / sizeof(HMODULE)); i++)
					{
						ReplaceIATEntryInImageImportTable(phModule[i], lpszDllName, pfnCurrent, pfnNew);
					}
				}
				delete[]phModule;
			}
...
BOOL CWininetHook::ReplaceIATEntryInImageImportTable(HANDLE hBaseAddress, LPCSTR lpszDllName, LPVOID pfnCurrent, LPVOID pfnNew)
{
	...
	while (pThunk->u1.Function)
	{
		...
		if ((*(DWORD_PTR*)lpAddr) == (DWORD_PTR)pfnCurrent)
		{
			...
			{
				DWORD oldProtect = 0;
				VirtualProtectEx(hProcess, lpAddr, sizeof(lpAddr), PAGE_EXECUTE_READWRITE, &oldProtect);
				// modify the address
				::WriteProcessMemory(hProcess, lpAddr, &pfnNew, sizeof(pfnNew), NULL);
				VirtualProtectEx(hProcess, lpAddr, sizeof(lpAddr), oldProtect, &oldProtect);
				CloseHandle(hProcess);
			}
			return TRUE;
		}
...
BOOL CWininetHook::ReplaceIATEntryInDelayImageImportTable(HANDLE hBaseAddress, LPCSTR lpszDllName, LPVOID pfnCurrent, LPVOID pfnNew)
{
	ASSERT(hBaseAddress && lpszDllName && pfnCurrent && pfnNew);
...
NTSTATUS WINAPI CWininetHook::_LdrGetProcedureAddress(IN HMODULE ModuleHandle, IN PANSI_STRING FunctionName OPTIONAL, IN WORD Oridinal OPTIONAL, OUT PVOID* FunctionAddress)
{
	...
	if (ntStatus == STATUS_SUCCESS)
	{
		...
		if (GetModuleFileName(ModuleHandle, tszPath, MAX_PATH))
		{
			...
			if (strFile.CompareNoCase(TEXT("WININET.dll")) == 0)
			{
				CHAR szFunName[1024] = { 0 };
				memcpy(szFunName, FunctionName->Buffer, FunctionName->Length);
#ifdef _DEBUG
				USES_CONVERSION;
				TRACE(TEXT("LdrGetProcedureAddress %s\n"), A2T(szFunName));
#endif				
				...
				TRY_GET_ADDRESS("InternetReadFile", s_pfnInternetReadFile, _InternetReadFile, PFN_InternetReadFile);
				TRY_GET_ADDRESS("InternetReadFileExA", s_pfnInternetReadFileExA, _InternetReadFileExA, PFN_InternetReadFileExA);
				TRY_GET_ADDRESS("InternetReadFileExW", s_pfnInternetReadFileExW, _InternetReadFileExW, PFN_InternetReadFileExW);
			}
		}
	}
...
HINTERNET WINAPI CWininetHook::_HttpOpenRequestW(__in HINTERNET hConnect, __in_opt LPCWSTR lpszVerb, __in_opt LPCWSTR lpszObjectName, __in_opt LPCWSTR lpszVersion, __in_opt LPCWSTR lpszReferrer, __in_z_opt LPCWSTR FAR* lplpszAcceptTypes, __in DWORD dwFlags, __in_opt DWORD_PTR dwContext)
{
	...
	{
		USES_CONVERSION;
		CMainFrame::GetRequestDlg().LogRequest(hConnect, hRequest, (lpszObjectName != NULL) ? W2T((LPWSTR)lpszObjectName) : NULL, (lpszVerb != NULL) ? W2T((LPWSTR)lpszVerb) : NULL);
	}
	return hRequest;
}
...
BOOL WINAPI CWininetHook::_InternetReadFile(_In_ HINTERNET hFile, _Out_ LPVOID lpBuffer, _In_ DWORD dwNumberOfBytesToRead, _Out_ LPDWORD lpdwNumberOfBytesRead)
{
	BOOL bRet = s_pfnInternetReadFile(hFile, lpBuffer, dwNumberOfBytesToRead, lpdwNumberOfBytesRead);
	if (bRet == TRUE)
	{
		CMainFrame::GetRequestDlg().LogHttpResponseData(lpBuffer, *lpdwNumberOfBytesRead);
	}
	return bRet;
}
BOOL WINAPI CWininetHook::_InternetReadFileExA(_In_ HINTERNET hFile, _Out_ LPINTERNET_BUFFERSA lpBuffersOut, _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext)
{
	BOOL bRet = s_pfnInternetReadFileExA(hFile, lpBuffersOut, dwFlags, dwContext);
	if (bRet == TRUE)
	{
		CMainFrame::GetRequestDlg().LogHttpResponseData(lpBuffersOut);
	}
	return bRet;
}
BOOL WINAPI CWininetHook::_InternetReadFileExW(_In_ HINTERNET hFile, _Out_ LPINTERNET_BUFFERSW lpBuffersOut, _In_ DWORD dwFlags, _In_opt_ DWORD_PTR dwContext)
{
	BOOL bRet = s_pfnInternetReadFileExW(hFile, lpBuffersOut, dwFlags, dwContext);
	if (bRet == TRUE)
	{
		CMainFrame::GetRequestDlg().LogHttpResponseData(lpBuffersOut);
	}
